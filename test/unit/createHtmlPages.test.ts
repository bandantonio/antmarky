...
describe('module createHtmlPages', () => {  
  test('Ensure the output directory contains generated static files (README in the default docs directory)', async () => {
    ...
    // const doesFolderExist = await directoryExists('public');
    // await expect(mock.bypass(() => doesFolderExist)).toBeFalsy();
  ...
  // test.skip('Ensure assets in the default docs directory are copied to the output directory', async () => {
  //   mock({
  //     'docs': {
  //       'unit-test.adoc': '== Unit test 1\nUnit test 1 file content',
  //     },
